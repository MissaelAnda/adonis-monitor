...
export class RequestMonitor extends Monitor<RequestType> {
  ...
  #format({ ctx, duration }: HttpRequestFinishedPayload) {
    const { request, response, route, params } = ctx
    // @ts-ignore: If not present its just undefined
    const auth = ctx.auth as Authenticator<Authenticators> | undefined
    return {
      duration: duration,
      auth: auth?.user
        ? {
      ...
      route: route
        ? {
            ...
            handler: formatHandler(
              typeof route!.handler === 'function' ? route!.handler : route!.handler.reference
            ),
            middleware: route!.middleware
              .all()
              .entries()
              .map(([middleware, _]) =>
                formatHandler(typeof middleware === 'function' ? middleware : middleware.name!)
              )
              .toArray(),
  ...
  #filterMonitorRequests(): EntryFilterFn<RequestType> {
    return (entry) => {
      ...
      return route?.name?.startsWith(monitor.routeName) || request.url.startsWith('/inertia')
  ...
  #truncateResponseBody(): EntryTransformerFn<RequestType> {
    return (payload) => {
      ...
      if (
        this.config.truncateResponseBody &&
        JSON.stringify(payload.response.body).length > this.config.responseBodyMaxSize
      ) {
